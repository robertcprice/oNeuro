...
- `oneuro-metal/src/whole_cell.rs` is still too large; chromosome/polymer dynamics, assembly/runtime-chemistry execution, and bootstrap/serialization boundaries still need to be split into subsystem modules so the runtime structure matches the biology layers
...
1. Continue splitting `oneuro-metal/src/whole_cell.rs` along authoritative biology/runtime boundaries so the runtime stops accumulating unrelated systems in one file. `local_chemistry.rs`, `spatial.rs`, and `membrane.rs` are now extracted; next modules are chromosome/polymer dynamics, assembly and named-complex execution, runtime chemistry/process execution, and bootstrap/serialization boundaries.
