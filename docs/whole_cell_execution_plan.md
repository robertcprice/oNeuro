...
- the remaining compatibility-only summary payloads and legacy serialization bridges are now concentrated in the last non-explicit restore, snapshot, or migration paths that still lack richer persisted payloads for MD-probe and scheduled local-chemistry detail, plus a few remaining expression or generic assembly carriers that still depend on compatibility summaries before persisted rollups can disappear entirely
