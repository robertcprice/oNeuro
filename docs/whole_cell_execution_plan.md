...
- the remaining compatibility-only summary payloads and legacy serialization bridges still preserve synchronized scalar state for non-explicit paths that do not yet carry richer explicit persisted state, instead of deriving everything from explicit state at the boundary
...
1. Remove the remaining compatibility-only summary payload rollups and legacy serialization bridges, so synchronized scalar summaries become boundary serialization or diagnostics only and not live execution state even on compatibility paths without bundle assets.
